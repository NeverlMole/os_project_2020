...
public final class ElevatorBank implements Runnable {
   ...
   */
  public static void selfTest() { new ElevatorTest().run(); }
  ...
  private class ElevatorManager implements ElevatorControls {
      ...
      for (int i = 0; i < numElevators; i++) {
        if (elevators[i].atNextFloor()) {
          ...
          if (elevators[i].atDestination()) {
            ...
            privilege.interrupt.schedule(Stats.ElevatorTicks, "elevator",
                                         interrupt);
          }
        }
      }
    ...
    void initialize() { controller.initialize(this); }
  ...
  private class RiderState implements RiderControls {
    ...
    public boolean pressFloorButton(int floor) {
      ...
      manager.postEvent(ElevatorEvent.eventFloorButtonPressed, floor, elevator,
                        true);
    ...
    void initialize() { rider.initialize(this, stops); }
