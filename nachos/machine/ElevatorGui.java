...
public final class ElevatorGui extends Frame {
  ...
  void openDoors(int elevator) { elevators[elevator].openDoors(); }

  void closeDoors(int elevator) { elevators[elevator].closeDoors(); }

  void setDirectionDisplay(int elevator, int direction) {
    elevators[elevator].setDirectionDisplay(direction);
  }

  void pressUpButton(int floor) { floors[floor].pressUpButton(); }

  void clearUpButton(int floor) { floors[floor].clearUpButton(); }

  void pressDownButton(int floor) { floors[floor].pressDownButton(); }

  void clearDownButton(int floor) { floors[floor].clearDownButton(); }
  ...
  private class ElevatorShaft extends Canvas {
    ...
    private void repaintElevator() {
      repaint(s, h * (numFloors - 1 - Math.max(floor, prevFloor)), w - 2 * s,
              h * 2);
    ...
    public void paint(Graphics g) {
      ...
      Rectangle e =
          new Rectangle(d.x, d.y + d.height, d.width, h - d.height - u);
