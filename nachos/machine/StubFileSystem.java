...
public class StubFileSystem implements FileSystem {
  ...
  private class FileRemover implements Runnable {
    public FileRemover(File f) { this.f = f; }

    public void run() { successful = f.delete(); }
  ...
  private static void reject(char c) { allowedFileNameCharacters[c] = false; }

  private static void allow(char c) { allowedFileNameCharacters[c] = true; }
