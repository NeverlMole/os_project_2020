...
public class Packet {
  ...
  public Packet(int dstLink, int srcLink, byte[] contents)
    ...
    if (dstLink < 0 || dstLink >= linkAddressLimit || srcLink < 0 ||
        srcLink >= linkAddressLimit || contents.length > maxContentsLength)
  ...
  public Packet(byte[] packetBytes) throws MalformedPacketException {
    ...
    if (packetBytes.length < headerLength ||
        packetBytes[0] != NetworkLink.networkID || packetBytes[1] < 0 ||
        packetBytes[1] >= linkAddressLimit || packetBytes[2] < 0 ||
        packetBytes[2] >= linkAddressLimit || packetBytes[3] < 0 ||
