...
func main() {
  ...
  go func(c chan int) {
    for {
      ...
      for _, address := range serverList{
        ...
        r, err := c.PushBlock(ctx, &pb.JsonBlockString{Json: "???Haha! This is a dirty block!!!"})
      }
      select {
      case <-c:
        wg.Done()
  ...
  for j := 1; j <= server_num; j++{
    assert(init_value1[1], init_value1[j], "Verify the balance")
    assert(init_value2[1], init_value2[j], "Verify the balance")
  }
  ...
  for i := 1; i <= 400; i++ {
    ...
    for j := 1; j <= server_num; j++{
      assert(init_value1[1] - int32(i), value1[j], "Verify the balance")
      assert(init_value2[1] + int32(i), value2[j], "Verify the balance")
    }
  ...
  c_stop <- 0
  wg.Wait()
  fmt.Println("Test Recovery Passed.")
}
