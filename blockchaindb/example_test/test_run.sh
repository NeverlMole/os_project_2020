...
for I in `seq 0 9`; do
	go run ./example_test/test_client.go -T=TRANSFER --from USER000$I --to USER0099 --value=5 --fee=1
done
sleep 10
echo "Check value: expecting value=995"
go run ./example_test/test_client.go -T=GET -user=USER0005
...
go run ./example_test/test_client.go -T=GET -user=USER0099
